...
class InitialStateBridge(Bridge):
  """A bridge that creates an initial decoder state based on the (last) output
...
class MeanInitialStateBridge(Bridge):
  """A bridge that creates an initial decoder state based on the mean outputs
  of the encoder. This state is created by passing the encoder outputs
  through an additional layer to match them to the decoder state size.
  The input function remains unmodified.
  ...
  def __init__(self, encoder_outputs, decoder_state_size, params, mode):
    super(MeanInitialStateBridge, self).__init__(encoder_outputs,
                                             decoder_state_size, params, mode)
  ...
  def _create(self):
    ...
    mean_outputs = tf.reduce_mean(self._bridge_input,1)
