...
class AttentionDecoder(RNNDecoder):
  ...
  def __init__(self,
    ...
    self.attention_fn_txt = attention_fn_txt
  ...
  def output_size(self):
    return AttentionDecoderOutput(
        ...
        cell_output=self.cell_rec2.output_size,
  ...
  def compute_output(self, inputs, cell_output, attention_scores, attention_context):

    """Computes the decoder outputs."""


    #get dropout parameters
    dropout_params = self.params["dropout_decoder"]
    ...
    deep_inputs = tf.nn.dropout(tf.concat([inputs, cell_output, attention_context], 1), dropout_params["dropout_deep_output_layer"])

    softmax_input = tf.contrib.layers.fully_connected(
        inputs=deep_inputs,
        num_outputs=self.cell_rec2.output_size,
  ...
  def step(self, time_, inputs, state, name=None):
    with tf.variable_scope("rec1"):
      cell_output, cell_state = self.cell_rec1(inputs, state)

    w_img = tf.contrib.layers.fully_connected(
        inputs=self.image_features,
        num_outputs=int(self.attention_values.shape[2]),
        activation_fn=tf.nn.tanh,
        scope="W_img")

    #w_img is of size 32,512
    #need to reshape to 32, 1, 512
    w_img = tf.expand_dims(w_img,1) # 31,1,512

    self.attention_values = tf.multiply(self.attention_values, w_img)


    attention_scores, attention_context = self.attention_fn_txt(
        query=cell_output,
        # keys=self.attention_keys,
        keys=self.attention_values,
        values=self.attention_values,
        values_length=self.attention_values_length)

    with tf.variable_scope("rec2"):
      cell_output, cell_state = self.cell_rec2(attention_context, cell_state)

    cell_output_new, logits = \
      self.compute_output(inputs,cell_output, attention_scores, attention_context)
