...
def _default_rnn_dropout_params():
  """Creates default parameters used by multiple RNN encoders.
  """
  return {
      "dropout_deep_output_layer": 1.0,
  }
...
class RNNDecoder(Decoder, GraphModule, Configurable):
  ...
  def __init__(self, params, mode, name):
    ...
    self.params["dropout_decoder"] = _toggle_dropout(self.params["dropout_decoder"], ["dropout_deep_output_layer"], mode)


    with tf.variable_scope("rec1"):
        self.cell_rec1 = training_utils.get_rnn_cell(**self.params["rnn_cell"])
    with tf.variable_scope("rec2"):
        self.cell_rec2 = training_utils.get_rnn_cell(**self.params["rnn_cell"])
  ...
  def default_params():
    return {
        ...
        "init_scale": 0.04,
        "dropout_decoder": _default_rnn_dropout_params(),
    }
