...
from seq2seq.contrib.training import basic_session_run_hooks
...
from seq2seq.contrib.learn.partial_restore_session import CheckpointRestorerHook
...
class BaseEstimator(
  ...
  def _train_model(self, input_fn, hooks):
    ...
    with self._graph.as_default() as g, g.device(self._device_fn):
      ...
      #restore some var from resnet checkpoiint
      def name_in_checkpoint(var):
          index = var.op.name.find('resnet_v1_50')
          if index != -1:
            return var.op.name.replace(var.op.name, var.op.name[index:])
          return var.op.name
      ...
      #     if ('resnet_v2_50' in var.name and "OptimizeLoss" not in var.name) or "global_step" in var.name:
      #         print(var.name, var.shape)


      variables_to_restore = {name_in_checkpoint(var): var
                              for var in slim.get_variables_to_restore()
                              if ('resnet_v1_50' in var.name and 'OptimizeLoss' not in var.name
                                  and 'ExponentialMovingAverage' not in var.name
                                  and "fully_connected" not in var.name)
                              or "global_step" in var.name}
      ...
      #     print(i.name, i.shape)  # i.name if you want just a name
      #
      # import sys
      # sys.exit()
      restore_var_hook = basic_session_run_hooks.CheckpointRestorerHook(checkpoint_file="models/resnet_v1_50.ckpt",
                                                                        var_list=variables_to_restore)

      chief_hooks.append(restore_var_hook)
