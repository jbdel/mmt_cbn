...
def batch_norm(inputs,
    ...
    with variable_scope.variable_scope(
        ...
        exclude_scopes = ["resnet_v1_50/conv1", "shortcut"] + exclude_scope


        scope_name = variable_scope.get_variable_scope().name
        exclude_cbn = any([(needle in scope_name) for needle in exclude_scopes])

        if(not exclude_cbn):
            #add FC for paper

            #need to concat fw and bw final states
            final_states = array_ops.concat(encoder_final_states,1)
            cbn_embedding = int(final_states.shape.as_list()[1]/2)
            num_output = params_shape.as_list()[0]
            initializer = RandomUniform(-1e-4, 1e-4)

            hidden_gamma = fully_connected(
                inputs=final_states,
                num_outputs=cbn_embedding,
                weights_initializer=initializer)

            delta_gamma = fully_connected(
                inputs=hidden_gamma,
            ...
            hidden_beta = fully_connected(
                inputs=final_states,
                num_outputs=cbn_embedding,
                weights_initializer=initializer)

            delta_beta = fully_connected(
                inputs=hidden_beta,
                num_outputs=num_output,
                activation_fn=None,
                weights_initializer=initializer,
...
def batch_normalization(x,
  ...
  with ops.name_scope(name, "batchnorm", [x, mean, variance, scale, offset]):
    ...
    inv *= scale
    expanded_inv = array_ops.reshape(inv, [-1, 1, 1, num_channels])
    expanded_mean = array_ops.reshape(mean, [-1, 1, 1, num_channels])
    expanded_betas = array_ops.reshape(offset, [-1, 1, 1, num_channels])
    return expanded_inv * (x - expanded_mean) + expanded_betas
