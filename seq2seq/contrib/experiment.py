...
class Experiment(tf.contrib.learn.Experiment):
  ...
  def _has_early_stop_reached(self, eval_result):
      """Determines if meteor hasnt improved over 10 updates."""
      #first check if none (first iter)
      if not eval_result:
        return False
      ...
      if meteor > self.best_meteor:
          ...
          tf.logging.info("New best meteor score:"+ str(self.best_meteor))
          with open("train/best-meteor.txt", "a+") as myfile:
              myfile.write(str(global_step) + '|' + str(meteor) + '|' + str(bleu) + "\n")
      else:
          self.early_stop_step += 1
          tf.logging.info("No best meteor score, early stop iter:"+ str(self.early_stop_step))
  ...
  def continuous_train_and_eval(self,
    ...
    while (not continuous_eval_predicate_fn or
      ...
      if self._has_early_stop_reached(eval_result):
        # Exits once max steps of training is satisfied.
        tf.logging.info("Early stop reached")
        break
      ...
      tf.logging.info("Training model for %s steps "+ str(train_steps_per_iteration))
      self._estimator.fit(
