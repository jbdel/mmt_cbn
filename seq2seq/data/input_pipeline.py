...
class TFRecordInputPipeline(InputPipeline):
  ...
  def make_data_provider(self, **kwargs):
    ...
    splitter_image = split_tokens_decoder.ProcessImage(
        image_feature_name="source_image",
    ...
    keys_to_features = {
        ...
        self.params["image_field"] : tf.FixedLenFeature((), tf.string, default_value="")
    ...
    items_to_handlers["source_image"] = tfexample_decoder.ItemHandlerCallback(
        keys=[self.params["image_field"]],
        func=lambda dict: splitter_image.decode(
            dict[self.params["image_field"]], ["source_image"])[0])
  ...
  @property
  def feature_keys(self):
    return set(["source_tokens", "source_len", "source_image"])

  @property
  def label_keys(self):
    #length image is len of feature map (14x14 = 196)
    return set(["target_tokens", "target_len"])
