...
from seq2seq.data.vgg_preprocessing import preprocess_for_eval, preprocess_for_train
...
class ProcessImage(data_decoder.DataDecoder):
  ...
  def decode(self, data, items):
    ...
    # if self.is_training == tf.contrib.learn.ModeKeys.TRAIN:
    if False:
      image = preprocess_for_train(image, self.height, self.width,
                                   self.resize_side_min, self.resize_side_max)
    else:
      image = preprocess_for_eval(image, self.height, self.width,
                                  self.resize_side_min)


    decoded_items[self.image_feature_name] = image

    return [decoded_items[_] for _ in items]

  def list_items(self):
    return [self.image_feature_name]
